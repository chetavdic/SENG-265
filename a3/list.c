 ...
 *
 */
node_t *new_node(char *subject, int statistic,char *icaocode) {
    assert(subject != NULL);
    assert(statistic != NULL);
    assert(icaocode!=NULL);
...
node_t *add_inorder(node_t * list, node_t *new) {
    ...
    for (curr = list; curr != NULL; curr = curr->next)
    {
        ...
        {
            prev = curr;
        }else if (new->statistic == curr->statistic){
            if (strcmp(curr->icaocode,new->icaocode)>0){
            prev = curr;
            }else{
                break;
            }
